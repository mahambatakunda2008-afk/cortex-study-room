...
/** Mozaik-native concurrent/reactive runtime seam for Cortex. */
...
const { initializeRuntime, resolveRuntime, join, sendMessage, runLoop } = runtime;
let initialized = false;

class WhenLearnerGoal extends SituationSpecification {
  constructor(private readonly humanId: string) { super(); }
...
class WhenPeerAnswers extends SituationSpecification {
  constructor(private readonly role: CortexRole, private readonly ids: Partial<Record<CortexRole, string>>) { super(); }
  isSatisfiedBy({ event, participant }: SituationContext): boolean {
...
export function createCortexMozaikRoom(goal: string) {
  if (!initialized) {
    initializeRuntime({ state: new CortexRuntimeState() });
    initialized = true;
  }

  resolveRuntime().state.goal = goal;
  ...
  const makeHandlers = (role: CortexRole): SituationHandler[] => [
    {
      ...
      processor: {
        apply({ event, participant }) {
          ...
          resolveRuntime().state.lastActivity[participant.getId()] = 'Started independent analysis';
    ...
    },
    {
      ...
      processor: {
        apply({ participant }) {
          if (!(participant instanceof Agent)) return;
          resolveRuntime().state.lastActivity[participant.getId()] = 'Reacting to peer output';
          runLoop(
            participant.getId(),
  ...
  join(human);
  Object.values(agents).forEach((agent) => join(agent));
