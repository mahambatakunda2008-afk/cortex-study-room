...
import {Activity,BrainCircuit,CheckCircle2,ChevronRight,CircleDot,GitBranch,Lightbulb,RotateCcw,Send,Sparkles,Target,TriangleAlert,Zap} from 'lucide-react';
 ...
 return <main>
  ...
  <section className="workspace">
   ...
   <div className="center"><div className="agents">{agents.map(a=><AgentCard key={a.id} a={a} state={state}/>)}</div>
    ...
    {state.knowledgeGraph.nodes.length>0&&<div className="graph panel"><div className="panelhead"><span>PREREQUISITE GRAPH</span><span className="live"><i/> causal</span></div><div className="graphchain">{state.knowledgeGraph.edges.slice(0,7).map((edge,i)=>{const from=state.knowledgeGraph.nodes.find(n=>n.id===edge.from);const to=state.knowledgeGraph.nodes.find(n=>n.id===edge.to);return <div className="edge" key={i}><span>{from?.name}</span><GitBranch size={13}/><strong>{to?.name}</strong></div>})}</div>{state.prerequisiteGap&&<div className="gap"><TriangleAlert size={15}/><span><b>PREREQUISITE TARGET:</b> {state.prerequisiteGap}</span></div>}</div>}
