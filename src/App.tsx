...
export default function App(){
 ...
 return <main>
  <header className="top">
   <div className="brand"><div className="logo"><BrainCircuit size={21}/></div><div><strong>CORTEX</strong><span>ADAPTIVE STUDY SYSTEM</span></div></div>
   <div className="status"><CircleDot size={12}/> SESSION <b>{state.phase==='complete'?'COMPLETE':state.phase==='running'?'ACTIVE':'READY'}</b></div>
  </header>
  ...
  <section className="workspace">
   <aside className="left panel">
    <div className="panelhead"><span>STUDY OBJECTIVE</span><Target size={16}/></div>
    <textarea value={goal} onChange={e=>setGoal(e.target.value)} disabled={state.phase==='running'}/>
    <button className="primary" onClick={start} disabled={state.phase==='running'}>{state.phase==='running'?<><Activity size={17}/> SESSION ACTIVE</>:<><Zap size={17}/> START SESSION</>}<ChevronRight size={17}/></button>
    <button className="ghost" onClick={()=>{setAnswer('');room.reset(goal)}}><RotateCcw size={15}/> Reset session</button>
    <div className="metric"><span>AGENT PROGRESS</span><b>{avg}%</b></div>
   ...
   <aside className="right panel">
    <div className="panelhead"><span>ACTIVITY LOG</span><span className="count">{state.events.length}</span></div>
    ...
    {state.plan&&<div className="plan"><small>NEXT STUDY STEP</small><p>{state.plan}</p></div>}
   </aside>
  </section>
...
function QuestionPanel({state,answer,setAnswer,submit,onPractice,onReframe}:{state:RoomState;answer:string;setAnswer:(v:string)=>void;submit:(value?:string)=>void;onPractice:()=>void;onReframe:()=>void}){
 ...
 return <div className="question panel">
  <div className="qrow"><div className="qtag">ASSESSMENT · {q.type.toUpperCase()} · ROUND {state.questionNumber}</div>{state.mastery>0&&<span className="masterybadge">MASTERY {state.mastery}%</span>}</div>
  <h3>{q.prompt}</h3>
  ...
  <div className="questionactions"><button className="ghost" onClick={onReframe}><Lightbulb size={15}/> Re-explain</button><button className="primary small" disabled={!correct} onClick={onPractice}><Zap size={15}/> {correct?'Continue practice':'Practice'}</button></div>
 </div>;
}
